...
from core.workflow import build_default_state, merge_state_defaults
...
class FileSessionRepository:
    ...
    def load(self, session_id: str | None) -> dict:
        ...
        if not session_file.exists():
            self.save(normalized, build_default_state(session_id=normalized))
        ...
        return merge_state_defaults(state, session_id=normalized)

    def save(self, session_id: str | None, state: dict) -> dict:
        normalized = normalize_session_id(session_id or state.get("session_id"))
        session_file = self.session_file(normalized)
        merged_state = merge_state_defaults(state, session_id=normalized)
